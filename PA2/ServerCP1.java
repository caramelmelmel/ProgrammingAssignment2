...
public class ServerCP1 {
	...
	public static void main(String[] args) {
		...
		try {
			privatekey = ServerCP1.getPrivateKey("private_key.der");
		...
		try {
			...
			while (!connectionSocket.isClosed()) {
				...
				if (packetType == 0) {
					...
					byte[] filename = readEncryptedFromClient(RSAdecryptcipher, fromClient);
					...
					block = RSAdecryptcipher.doFinal(encryptedBlock);
